...
void leerMallayCondiciones(mesh &m){
    char filename[14];
    ifstream file;
    float l, tau, kappa, lambda, ny,  psi,  alpha,  delta,  eta;
    ...
    file >> l >> tau >> kappa >> lambda >> ny >>  psi >>  alpha >>  delta >>  eta;
    file >> nnodes >> neltos >> ndirich_u >> ndirich_p;

    
    m.setParameters(l, tau, kappa, lambda, ny,  psi,  alpha,  delta,  eta);
